...
function show_node(query) {
  ...
  vis.selectAll(".node")
    .classed("marked", function(d) {
      return d.name.toLowerCase().indexOf(query.toLowerCase()) >= 0 || d.id.toLowerCase().indexOf(query.toLowerCase()) >= 0
    })
}
...
//          .attr("id", "nodestat")
//          .text("Statistik")
//          .style({'text-decoration': 'none', 'font-size': '25%'})
...
function update() {
  ...
  node.attr("class", function(d) {
        ...
        //  s.push(["legacy"])
        if (d.firmware) {
          if (d.firmware.substr(0,3) < "1.2" || d.firmware.substr(0,3) > "100")
            s.push(["legacy"])
        }
        //if (d.vpns.length > 0)
        //  s.push(["uplink"])
